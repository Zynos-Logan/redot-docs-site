...
provides the [Parallax2D](/docs/Classes/parallax2d) node to achieve this effect. It can still be easy to get tripped
up though, so this page provides in-depth descriptions of some properties and how to fix some common mistakes.

:::note

This page only covers how to use [Parallax2D](/docs/Classes/parallax2d). This node is still experimental, so the
implementation might change in future versions of Redot. However, it is still recommended to use over the
[ParallaxLayer](/docs/Classes/parallaxlayer) and [ParallaxBackground](/docs/Classes/parallaxbackground) nodes. 

:::

## Scroll scale

The backbone of the parallax effect is the [scroll_scale ](/docs/Classes/parallax2d_property_scroll_scale) property.
...
The scene above is comprised of five layers. Some good [scroll_scale ](/docs/Classes/parallax2d_property_scroll_scale)
values might be:
...
<video src="/video/2d_parallax_scroll_scale.webm" controls autoplay loop muted>
  A scene with five layers scrolling at different speeds
</video>

## Infinite repeat

[Parallax2D](/docs/Classes/parallax2d) provides a bonus effect that gives textures the illusion of repeating infinitely.
[repeat_size](/docs/Classes/parallax2d_property_repeat_size) tells the node to snap its position forward or back when the
...
size or larger than your viewport **before** setting the [repeat_size](/docs/Classes/parallax2d_property_repeat_size). If
...
[repeat_size](/docs/Classes/parallax2d_property_repeat_size) to the size of our image, the infinite repeat effect doesn't
scroll properly because the original texture doesn't cover the viewport. If we set the
[repeat_size](/docs/Classes/parallax2d_property_repeat_size) to the size of the viewport, we have a large gap. What can we
do?
...
[Viewport Width](/docs/Classes/ProjectSettings_property_display/window/size/viewport_width)
and [Viewport Height](/docs/Classes/ProjectSettings_property_display/window/size/viewport_height)
...
larger to fit your screen. Set the [scale](/docs/Classes/node2d_property_scale) of the [Parallax2D](/docs/Classes/parallax2d),
and all child textures scale with it.

#### Scale the child nodes

Similar to scaling the [Parallax2D](/docs/Classes/parallax2d), you can scale your [Sprite2D](/docs/Classes/sprite2d) nodes to
be large enough to cover the screen. Keep in mind that some settings like
[Parallax2D.repeat_size](/docs/Classes/parallax2d_property_repeat_size) and
[Sprite2D.region_rect](/docs/Classes/sprite2d_property_region_rect) do not take scaling into account, so it's necessary to
...
[Sprite2D](/docs/Classes/sprite2d) you'd like to repeat, but is too small, you can do the following to repeat it:

- set [texture_repeat](/docs/Classes/canvasitem_property_texture_repeat) to [CanvasItem.TEXTURE_REPEAT_ENABLED](/docs/Classes/canvasitem_constant_TEXTURE_REPEAT_ENABLED)
- set [region_enabled](/docs/Classes/sprite2d_property_region_enabled) to ``true``
- set the [region_rect](/docs/Classes/sprite2d_property_region_rect) to a multiple of the size of your texture large enough to cover the viewport.
...
``(0,0)`` and expands down and to the right to the size of the [repeat_size](/docs/Classes/parallax2d_property_repeat_size)
value.
...
Increasing [repeat_times](/docs/Classes/parallax2d_property_repeat_times) technically *would* work in some scenarios, but
...
[Parallax2D.repeat_size](/docs/Classes/parallax2d_property_repeat_size) is set correctly, it should look something like
...
(determined by [repeat_size](/docs/Classes/parallax2d_property_repeat_size)), and when it reaches what's inside the yellow
...
[Parallax2D](/docs/Classes/parallax2d) comes with a [scroll_offset](/docs/Classes/parallax2d_property_scroll_offset) property
used to offset where the infinite repeat canvas starts. As an example, if your image is 288x208, setting
the [scroll_offset](/docs/Classes/parallax2d_property_scroll_offset) to ``(-144,0)`` or ``(144,0)`` allows it to begin
...
occur when we zoom out by setting the [Camera2D.zoom](/docs/Classes/camera2d_property_zoom) to ``(0.5, 0.5)``:
...
[repeat_times](/docs/Classes/parallax2d_property_repeat_times) can help out. Setting a value of ``3`` (one extra
...
[repeat_size](/docs/Classes/parallax2d_property_repeat_size). The
[repeat_times](/docs/Classes/parallax2d_property_repeat_times) would automatically add a repeat above and below as well.
...
the [Viewport.world_2d](/docs/Classes/viewport_property_world_2d) of the first SubViewport to the second, so they have a
...
[SubViewport](/docs/Classes/subviewport). Here's how a setup looks for a two player game:
...
- Leave all parallax nodes at their default [visibility_layer](/docs/Classes/canvasitem_property_visibility_layer) of 1.
- Set the first SubViewport's [canvas_cull_mask](/docs/Classes/viewport_property_canvas_cull_mask) to only layers 1 and 2.
- Do the same for the second SubViewport but use layers 1 and 3.
- Give your parallax nodes in the first SubViewport a common parent and set its [visibility_layer](/docs/Classes/canvasitem_property_visibility_layer) to 2.
- Do the same for the second SubViewport's parallax nodes, but use a layer of 3.

How does this work? If a canvas item has a [visibility_layer](/docs/Classes/canvasitem_property_visibility_layer) that
doesn't match the SubViewport's [canvas_cull_mask](/docs/Classes/viewport_property_canvas_cull_mask), it will hide all
children, even if they do. We use this to our advantage, letting the SubViewports cut off rendering of parallax nodes
whose parent doesn't have a supported [visibility_layer](/docs/Classes/canvasitem_property_visibility_layer).

## Previewing in the editor

Prior to 4.3, the recommendation was to place every layer in their own
[ParallaxBackground](/docs/Classes/parallaxbackground), enable the
[follow_viewport_enabled](/docs/Classes/canvaslayer_property_follow_viewport_enabled) property, and scale the individual
layer. This method has always been tricky to get right, but is still achievable by using a
[CanvasLayer](/docs/Classes/canvaslayer) instead of a [ParallaxBackground](/docs/Classes/parallaxbackground).
