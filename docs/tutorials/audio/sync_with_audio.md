...
As mentioned before, If you call [AudioStreamPlayer.play()](/docs/Classes/AudioStreamPlayer_method_play), sound will not begin immediately, but when the audio thread processes the next chunk.

This delay can't be avoided but it can be estimated by calling [AudioServer.get_time_to_next_mix()](/docs/Classes/AudioServer_method_get_time_to_next_mix).

The output latency (what happens after the mix) can also be estimated by calling [AudioServer.get_output_latency()](/docs/Classes/AudioServer_method_get_output_latency).
