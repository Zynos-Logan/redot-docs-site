...
<TabItem value="c____h_file" label="C++ .H File">
...
<TabItem value="c____cpp_file" label="C++ .CPP File">
...
[Thread.wait_to_finish()](/docs/Classes/Thread_method_wait_to_finish), which will
...
when modified. Always use a [Mutex](/docs/Classes/Mutex) when accessing
a piece of data from different threads.

When calling [Mutex.lock()](/docs/Classes/Mutex_method_lock), a thread ensures that
...
[Mutex.unlock()](/docs/Classes/Mutex_method_unlock), the other threads will be
...
For this, [Semaphores](/docs/Classes/Semaphore) are used. The function
[Semaphore.wait()](/docs/Classes/Semaphore_method_wait) is used in the thread to
suspend it until some data arrives.

The main thread, instead, uses
[Semaphore.post()](/docs/Classes/Semaphore_method_post) to signal that data is
