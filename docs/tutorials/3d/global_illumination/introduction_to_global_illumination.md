...
(SSAO) can be enabled in the [environment ](doc_environment_and_post_processing)
...
can be enabled in the [environment ](doc_environment_and_post_processing) settings.
...
- For desktop games, it's a good idea to start with [SDFGI ](doc_using_sdfgi)
  ...
  graphics, consider adding an option to disable SDFGI or [VoxelGI ](doc_using_voxel_gi) in your game's settings. SDFGI can be disabled in the
...
[Global Illumination demo project ](https://github.com/redot-engine/redot-demo-projects/tree/master/3d/global_illumination)_.
