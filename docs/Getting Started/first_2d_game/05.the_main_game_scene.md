...
Create a new scene and add a [Node](/docs/Classes/Node) named ``Main``.
...
- [Timer](/docs/Classes/Timer) (named ``MobTimer``) - to control how often mobs
  spawn
- [Timer](/docs/Classes/Timer) (named ``ScoreTimer``) - to increment the score
  every second
- [Timer](/docs/Classes/Timer) (named ``StartTimer``) - to give a delay before
  starting
- [Marker2D](/docs/Classes/Marker2D) (named ``StartPosition``) - to indicate
...
location on the edge of the screen. Add a [Path2D](/docs/Classes/Path2D) node
...
Now that the path is defined, add a [PathFollow2D](/docs/Classes/PathFollow2D)
...
:::info
Why ``PI``? In functions requiring angles, Redot uses *radians*,
