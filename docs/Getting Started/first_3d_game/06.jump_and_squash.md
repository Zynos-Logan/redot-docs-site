...
the [CharacterBody3D](/docs/Classes/CharacterBody3D) class and are related to
``move_and_slide()``.

``get_slide_collision()`` returns a
[KinematicCollision3D](/docs/Classes/KinematicCollision3D) object that holds
...
The method ``is_in_group()`` is available on every [Node](/docs/Classes/Node).
...
When using C#, Redot will create the appropriate events automatically for all Signals ending with `EventHandler`, see [C# Signals](../../tutorials/scripting/c_sharp/c_sharp_signals.md).
