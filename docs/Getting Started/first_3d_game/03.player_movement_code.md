...
These are common properties for a moving body. The ``target_velocity`` is a [3D vector](class_Vector3)
...
We compute the direction the ``$Pivot`` is looking by creating a [Basis](class_Basis)
...
smooth it out for you. It uses the *velocity* value native to the [CharacterBody3D](class_CharacterBody3D)
...
*Add Child Node*. Create a new [Marker3D](class_Marker3D), and name it ``CameraPivot``.
Select ``CameraPivot`` and add a child node [Camera3D](class_Camera3D) to it.
...
On the bottom view, select your [Camera3D](class_Camera3D) and turn on camera
