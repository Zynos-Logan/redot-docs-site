...
These are common properties for a moving body. The ``target_velocity`` is a [3D vector](/docs/Classes/Vector3)
...
``_physics_process()``, see [doc_idle_and_physics_processing](../../tutorials/scripting/idle_and_physics_processing.md).
...
We compute the direction the ``$Pivot`` is looking by creating a [Basis](/docs/Classes/Basis)
...
smooth it out for you. It uses the *velocity* value native to the [CharacterBody3D](/docs/Classes/CharacterBody3D)
...
*Add Child Node*. Create a new [Marker3D](/docs/Classes/Marker3D), and name it ``CameraPivot``.
Select ``CameraPivot`` and add a child node [Camera3D](/docs/Classes/Camera3D) to it.
...
On the bottom view, select your [Camera3D](/docs/Classes/Camera3D) and turn on camera
