...
In the main scene, add a new child node [Control](/docs/Classes/Control) to ``Main`` and name it
``UserInterface``. Ensure you are on the 2D screen, where you can edit your User Interface (UI).

Add a [Label](/docs/Classes/Label) node and name it ``ScoreLabel``
...
creating and editing themes, see [doc_gui_skinning](../../tutorials/ui/gui_skinning.md).
...
You can learn more about string formatting here: [doc_gdscript_printf](../../tutorials/scripting/gdscript/gdscript_format_string.md).
...
child node [ColorRect](/docs/Classes/ColorRect), and name it ``Retry``. This node fills a
...
Next, add a [Label](/docs/Classes/Label) as a child of ``Retry`` and give it the *Text*
...
another feature in Redot: [autoloads](../../tutorials/scripting/singletons_autoload.md).

To play audio, all you need to do is add an [AudioStreamPlayer](/docs/Classes/AudioStreamPlayer) node to your
...
Click the *Other Node* button to create an [AudioStreamPlayer](/docs/Classes/AudioStreamPlayer) and rename it to
``MusicPlayer``.
