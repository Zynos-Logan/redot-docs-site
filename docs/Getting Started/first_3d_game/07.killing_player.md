...
kinds of collisions. Instead, though, we will use an [Area3D](/docs/Classes/Area3D) node, which
works well for hitboxes.

## Hitbox with the Area node

Head back to the ``player.tscn`` scene and add a new child node [Area3D](/docs/Classes/Area3D). Name it
``MobDetector``
Add a [CollisionShape3D](/docs/Classes/CollisionShape3D) node as a child of it.
...
The *MobDetector* will emit ``body_entered`` when a [CharacterBody3D](/docs/Classes/CharacterBody3D) or a
[RigidBody3D](/docs/Classes/RigidBody3D) node enters it. As it only masks the "enemies" physics
