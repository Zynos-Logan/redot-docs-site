...
Create a scene with, once again, a [CharacterBody3D](/docs/Classes/CharacterBody3D) node as its root. Name it
``Mob``. Add a child node [Node3D](/docs/Classes/Node3D), name it ``Pivot``. And drag and drop
...
Add a [CollisionShape3D](/docs/Classes/CollisionShape3D).
...
[VisibleOnScreenNotifier3D](/docs/Classes/VisibleOnScreenNotifier3D), and we're going to use it to destroy our mobs.
...
about that here: [doc_gdscript_basics_memory_management](doc_gdscript_basics_memory_management).

:::

Select the ``Mob`` node and add a child node [VisibleOnScreenNotifier3D](/docs/Classes/VisibleOnScreenNotifier3D). Another
...
connect our [VisibleOnScreenNotifier3D](/docs/Classes/VisibleOnScreenNotifier3D) node's ``screen_exited`` signal to the ``Mob``.
...
Select the [VisibleOnScreenNotifier3D](/docs/Classes/VisibleOnScreenNotifier3D) node and on the right side of the interface,
