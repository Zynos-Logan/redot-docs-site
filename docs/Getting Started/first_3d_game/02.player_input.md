...
Add a [Node3D](class_Node3D) node as a child of ``Player`` and name it ``Pivot``
...
[CollisionShape3D](class_CollisionShape3D). In the *Inspector*, on the *Shape* property, add a new [SphereShape3D](class_SphereShape3D).
