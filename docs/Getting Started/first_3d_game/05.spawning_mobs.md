...
[PathFollow3D](/docs/Classes/PathFollow3D) node to sample random locations on it.
...
Select the [Camera3D](/docs/Classes/Camera3D) node and click the *Preview* checkbox in the bottom
viewport.
...
Let's add the placeholder meshes. Add a new [Node3D](/docs/Classes/Node3D) as a child of the
``Main`` node and name it ``Cylinders``. We'll use it to group the cylinders. Select ``Cylinders`` and add a child node [MeshInstance3D](/docs/Classes/MeshInstance3D)
...
In the *Inspector*, expand the *Material* section and assign a [StandardMaterial3D](/docs/Classes/StandardMaterial3D) to slot *0*.
...
Add a child node [Path3D](/docs/Classes/Path3D) to ``Main`` node. In the toolbar, four icons appear. Click
...
To sample random positions on it, we need a [PathFollow3D](/docs/Classes/PathFollow3D) node. Add a
[PathFollow3D](/docs/Classes/PathFollow3D) as a child of the ``Path3D``. Rename the two nodes to ``SpawnLocation`` and
...
Add a new [Timer](/docs/Classes/Timer) node as a child of ``Main``. Name it ``MobTimer``.
