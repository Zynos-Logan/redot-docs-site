...
[PathFollow3D](class_PathFollow3D) node to sample random locations on it.
...
Select the [Camera3D](class_Camera3D) node and click the *Preview* checkbox in the bottom
viewport.
...
Let's add the placeholder meshes. Add a new [Node3D](class_Node3D) as a child of the
``Main`` node and name it ``Cylinders``. We'll use it to group the cylinders. Select ``Cylinders`` and add a child node [MeshInstance3D](class_MeshInstance3D)
...
Add a child node [Path3D](class_Path3D) to ``Main`` node. In the toolbar, four icons appear. Click
...
To sample random positions on it, we need a [PathFollow3D](class_PathFollow3D) node. Add a
[PathFollow3D](class_PathFollow3D) as a child of the ``Path3D``. Rename the two nodes to ``SpawnLocation`` and
