...
Building upon the previous lesson, [doc_scripting_first_script](scripting_first_script.md), let's look
...
1. The built-in input callbacks, mainly ``_unhandled_input()``. Like
   ...
   more about input callbacks, see [doc_inputevent](../../tutorials/inputs/inputevent.md).
...
In the next lesson, [doc_signals](signals.md), we'll build upon the relationship between
